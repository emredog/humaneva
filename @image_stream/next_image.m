function [this, fname, img, map] = next_image(this); 
% NEXT_IMAGE Accesor function allows referencing stream object IMAGE_STREAM.
% 
% [...] = NEXT_IMAGE(...) reads the next grayscale or color image from the 
% image stream specified by the IMAGE_STREAM object. See the reference 
% page, or the output of the constructor IMAGE_STREAM, for a list of 
% supported options.  
%
% Syntax:
%   [THIS, FNAME] = next_image(THIS);
%   [THIS, FNAME, IMG] = next_image(THIS);
%   [THIS, FNAME, IMG, MAP] = next_image(THIS);
%
% Input:
